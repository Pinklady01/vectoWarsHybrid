...
GameStateDatasetSamplerService* gameStateService = NULL;
HybridInputPredictionStrategyService* inputPredictionStrategyService = NULL;
HybridGGPOServiceProvider* serviceProvider = NULL;
...
VectorWar_Init(HWND hwnd, unsigned short localport, int num_players, GGPOPlayer* players, int num_spectators)
{
	...
	gameStateService = new GameStateDatasetSamplerService();
	inputPredictionStrategyService = new HybridInputPredictionStrategyService();

	serviceProvider = new HybridGGPOServiceProvider(gameStateService, NULL);

#if defined(SYNC_TEST)
	result = ggpo_start_synctest(&ggpo, &cb, "vectorwar", num_players, sizeof(int), 1);
#else
	result = ggpo_start_hybrid_session(&ggpo, &cb, serviceProvider, "vectorwar", num_players, sizeof(int), localport, true);
#endif
...
VectorWar_Exit()
{
	...
	delete inputPredictionStrategyService;
	inputPredictionStrategyService = NULL;
